...
class FPN(torch.nn.Module):
    def __init__(self,chan):
        ...
        self.act_1 =torch.nn.LeakyReLU(0.1)
        ...
        self.con1_1_first=torch.nn.Conv2d(chan,int(chan/2),(1,1),padding=0)#横向的1*1调整通道
    ...
    def forward(self,x):
        ...
        y3_out=y3#通道为16
        y2_out = torch.cat((self.upsample_2(y3_out), self.con1_1_second(y2)), 1)#通道为24
        y1_out = torch.cat((self.upsample_1(y2_out), self.con1_1_first(y1)), 1)#通道为32
        return y1_out,y2_out,y3_out

class Pred(torch.nn.Module):
    def __init__(self,in_size):
        #CA层与Pred模块合并，下面为注意力机制模块
